    ...
    switch(n->t){
        ...
        case POW: {
            ...
            if(g_const){
                ...
                double c = g->v;
                if(c == 0.0) return num(0);         
                if(c == 1.0) return der(f);         
                if(c == 2.0)                        
            ...
            N* dlnf  = bin(DIV, der(f), f);               
            N* term1 = bin(MUL, der(g), un(LN, f));     
            N* term2 = bin(MUL, g,      dlnf);           
...
static double run(const char*fs, const char*gs, double a, Side side, int vb){
    ...
    LH:;{
        ...
        for(int iter=1; iter<=MAX_LH; iter++){
            ...
            if(!hd || !(dfa_z&&dga_z) && !(dfa_i&&dga_i)) break; 
            cf=df; cg=dg;
        }
    ...
    NUM_EST:;{
        ...
        double Lm = numeric_side(f,g,a,0);  
        double Lp = numeric_side(f,g,a,1);  
